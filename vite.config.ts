...
const appEnvMod = await optionalMod<{ appEnvPlugin: () => Plugin }>(
  "scripts/app-env-plugin.mjs",
);
const migrationMod = await optionalMod<{ isMigrationFile: (name: string) => boolean }>(
  "scripts/migration-plan.mjs",
);

function isMigrationFile(name: string): boolean {
  return migrationMod?.isMigrationFile?.(name) ?? false;
}
...
export default defineConfig(({ command, isPreview }) => ({
  ...
  plugins: [
    ...
    appEnvMod?.appEnvPlugin() ?? noopPlugin("app-env-optional"),
    // PWA head + ?install=1 tutorial page; runs before Start/Nitro.
    grokPwaMod?.grokPwaPlugin() ?? noopPlugin("grok-pwa-optional"),
    tailwindcss(),
    tanstackStart(),
    ...(command === "build" || isPreview
      ? [
          nitro({
            ...
            serverDir: existsSync(join(ROOT, "server")) ? "./server" : false,
          }),
        ]
      : []),
    viteReact(),
